...
void oled_init (void){
	...
	for (uint8_t tmp = 0; tmp< sizeof(ssd1306_init_cmd); tmp++) {
		i2cSend(pgm_read_byte(&ssd1306_init_cmd[tmp]));
	}
	i2cStop();
    oled_clear_screen();
}
...
void oled_gotoxy (uint8_t x, uint8_t y){
    ...
    i2cStart();
    i2cSend(COMMAND);
    i2cSend(SET_PAGE_START|y);
		i2cSend(SET_COLUMN_ADDRESS);
    i2cSend(x * 8);
		i2cSend(SET_DISPLAY_START_LINE|0x3F);
    i2cStop();
}
//***************************************************************************************
...
void oled_write_char (char c){
	if(font_size != 0){
		...
		i2cSend(DATA);
		for (uint8_t count = 0; count < 8; count++){
			i2cSend(pgm_read_byte(&font[(unsigned char)c][count]));
		}
		i2cStop();
		oled_x +=1;
	}
